...
async def test_prompt_above_threshold_triggers_consolidation(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("nanobot.agent.memory.store.estimate_message_tokens", lambda _message: 500)
...
async def test_prompt_above_threshold_archives_until_next_user_boundary(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("nanobot.agent.memory.store.estimate_message_tokens", lambda message: token_map[message["content"]])
...
async def test_consolidation_loops_until_target_met(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("nanobot.agent.memory.store.estimate_message_tokens", lambda _m: 100)
...
async def test_preflight_consolidation_before_llm_call(tmp_path, monkeypatch) -> None:
    ...
    loop.sessions.save(session)
    monkeypatch.setattr("nanobot.agent.memory.store.estimate_message_tokens", lambda _m: 500)
